...
describe('concierge module', () => {
  it('covers repository, service and controller', async () => {
    ...
    const repository = new ConciergeMessageRepository({ save, find, findOne, exists } as never);
    ...
    expect(await repository.hasGuestMessage('stay_001')).toBe(false);

    const stayRepository = { findById: jest.fn().mockResolvedValue({ id: 'stay_001' }) };
    const queueService = { publish: jest.fn() };
    const realtimeService = { publishMessage: jest.fn() };
    const service = new ConciergeService(stayRepository as never, repository, queueService as never, realtimeService as never);
    const session = { stayId: 'stay_001', hotelId: 'hotel_001' };
    ...
    expect(realtimeService.publishMessage).toHaveBeenCalledTimes(2);
    expect((await service.createMessage('stay_001', { text: 'Novamente' }, session as never)).reply).toBeNull();
    expect(realtimeService.publishMessage).toHaveBeenCalledTimes(3);
