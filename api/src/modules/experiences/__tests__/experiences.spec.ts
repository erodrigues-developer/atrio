...
describe('experiences module', () => {
  it('covers repository, service and controller', async () => {
    const collectionRepository = { find: jest.fn().mockResolvedValue([{ publicId: 'today', title: 'Today', description: 'Desc', featured: true }]), findOne: jest.fn().mockResolvedValue({ publicId: 'today', title: 'Today', description: 'Desc', featured: true }) };
    const collectionItemRepository = { find: jest.fn().mockResolvedValue([{ experience: { publicId: 'sunset-dinner', title: 'Sunset', description: 'Desc', category: 'Gastronomia', timeLabel: 'Hoje', priceLabel: 'Sob consulta', badge: 'Featured', imageUrl: 'url' } }]) };
    const experienceRepositoryInner = { findOne: jest.fn().mockResolvedValue({ publicId: 'sunset-dinner', title: 'Sunset', description: 'Desc', category: 'Gastronomia', timeLabel: 'Hoje', priceLabel: 'Sob consulta', badge: 'Featured', imageUrl: 'url', durationLabel: '2h', availabilityLabel: 'Hoje', locationLabel: 'Restaurante', locationDescription: 'Detalhes', included: ['Mesa'], policy: 'Policy' }) };
    const slotRepository = { find: jest.fn().mockResolvedValue([{ publicId: 'slot_001', date: '2026-06-13', dayLabel: 'Hoje', dateLabel: '13 jun', time: '18:30', startsAt: new Date('2026-06-13T21:30:00.000Z'), isAvailable: true }]), findOne: jest.fn(), save: jest.fn() };
    ...
    expect((await repository.findCollectionById('today'))?.publicId).toBe('today');
    expect((await repository.findExperienceById('sunset-dinner'))?.publicId).toBe('sunset-dinner');
