...
export class AdminExperiencesService {
  ...
  async listExperiences() {
    const experiences = await this.experienceRepository.find({ order: { title: 'ASC' } });
    return experiences.map((experience) => this.mapExperience(experience));
  }

  async createExperience(session: AdminSessionContext, input: UpsertAdminExperienceDto) {
    ...
    await this.record(session, 'experience.create', 'experience', saved.publicId, `created experience ${saved.title}`);
    return this.mapExperience(saved);
  }

  async updateExperience(session: AdminSessionContext, experienceId: string, input: UpsertAdminExperienceDto) {
    const experience = await this.getRequiredExperience(experienceId);
    this.assignExperience(experience, input);

    const saved = await this.experienceRepository.save(experience);
    await this.record(session, 'experience.update', 'experience', saved.publicId, `updated experience ${saved.title}`);
    return this.mapExperience(saved);
  }

  async listCollections() {
    const collections = await this.collectionRepository.find({ order: { featured: 'DESC', title: 'ASC' } });
    return collections.map((collection) => this.mapCollection(collection));
  }

  async createCollection(session: AdminSessionContext, input: UpsertAdminCollectionDto) {
    ...
    await this.record(session, 'experience_collection.create', 'experience_collection', saved.publicId, `created collection ${saved.title}`);
    return this.mapCollection(saved);
  }

  async updateCollection(session: AdminSessionContext, collectionId: string, input: UpsertAdminCollectionDto) {
    ...
    const saved = await this.collectionRepository.save(collection);
    await this.record(session, 'experience_collection.update', 'experience_collection', saved.publicId, `updated collection ${saved.title}`);
    return this.mapCollection(saved);
  }

  async linkExperience(session: AdminSessionContext, collectionId: string, input: LinkExperienceToCollectionDto) {
    ...
    await this.record(session, 'experience_collection.item.upsert', 'experience_collection', collectionId, `linked experience ${input.experienceId}`);
    return { id: saved.publicId, collectionId: saved.collectionId, experienceId: saved.experienceId, position: saved.position };
  }

  async listSlots(experienceId: string) {
    await this.getRequiredExperience(experienceId);
    const slots = await this.slotRepository.find({ where: { experienceId }, order: { startsAt: 'ASC' } });
    return slots.map((slot) => this.mapSlot(slot));
  }

  async createSlot(session: AdminSessionContext, experienceId: string, input: CreateAdminExperienceSlotDto) {
    await this.getRequiredExperience(experienceId);
    ...
    slot.dayLabel = new Intl.DateTimeFormat('pt-BR', { weekday: 'short', timeZone: 'UTC' }).format(startsAt);
    slot.dateLabel = new Intl.DateTimeFormat('pt-BR', { day: '2-digit', month: '2-digit', timeZone: 'UTC' }).format(startsAt);
    ...
    await this.record(session, 'experience_slot.create', 'experience_availability_slot', saved.publicId, `created slot for ${experienceId}`);
    return this.mapSlot(saved);
  }

  async updateSlot(session: AdminSessionContext, experienceId: string, slotId: string, input: UpdateAdminExperienceSlotDto) {
    await this.getRequiredExperience(experienceId);
    const slot = await this.slotRepository.findOne({ where: { publicId: slotId, experienceId } });
    ...
    slot.isAvailable = input.isAvailable;
    const saved = await this.slotRepository.save(slot);
    await this.record(session, 'experience_slot.update', 'experience_availability_slot', saved.publicId, `updated slot ${slotId}`);
    return this.mapSlot(saved);
  }

  async listReservations(session: AdminSessionContext, query: { status?: string; search?: string }) {
    ...
    if (query.status) {
      builder.andWhere('reservation.status = :status', { status: query.status });
    }

    if (query.search) {
      builder.andWhere(
        new Brackets((qb) => {
          qb.where('reservation.title ILIKE :search', { search: `%${query.search}%` })
            .orWhere('stay.room_number ILIKE :search', { search: `%${query.search}%` })
    ...
    return rows.map((row) => ({ ...row, scheduledAt: new Date(row.scheduledAt).toISOString() }));
  }

  async createReservation(session: AdminSessionContext, input: CreateAdminReservationDto) {
    ...
    const slot = await this.slotRepository.findOne({ where: { publicId: input.slotId, experienceId: input.experienceId } });

    if (!slot || !slot.isAvailable) {
      throw new ApiException(409, 'SLOT_UNAVAILABLE', 'Experience availability slot is unavailable.');
    }
    ...
    await this.record(session, 'reservation.create', 'reservation', saved.publicId, `created reservation ${saved.publicId}`);

    return this.mapReservation(saved, stay.roomNumber, `${stay.guest.firstName} ${stay.guest.lastName}`);
  }

  async updateReservationStatus(session: AdminSessionContext, reservationId: string, input: UpdateAdminReservationStatusDto) {
    const reservation = await this.reservationRepository
      .createQueryBuilder('reservation')
      .innerJoinAndMapOne('reservation.stay', Stay, 'stay', 'stay.public_id = reservation.stay_id')
    ...
    const saved = await this.reservationRepository.save(reservation);
    await this.record(session, 'reservation.status.update', 'reservation', saved.publicId, `updated reservation ${saved.publicId}`);
  ...
  private async getRequiredStay(session: AdminSessionContext, stayId: string) {
    const stay = await this.stayRepository.findOne({ where: { publicId: stayId, hotelId: session.hotelId }, relations: { guest: true } });
  ...
  private assignExperience(experience: Experience, input: UpsertAdminExperienceDto) {
  ...
  private mapReservation(reservation: Reservation, roomNumber: string, guestName: string) {
    return {
